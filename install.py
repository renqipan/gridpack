...
def main(args):
  ...
  #download and compile
    ...
    with cd(JHUbasedir):
      ...
      subprocess.check_call(["tar", "xzf", "JHUGenerator.tar.gz"])
      os.remove("JHUGenerator.tar.gz")
#      shutil.move("JHUGenerator."+JHUGenversion+"/*","./")
      src = "JHUGenerator."+JHUGenversion+"/"
      dest = './'
      files = os.listdir(src)
      for f in files:
         shutil.move(src + f, dest)
    # shutil.copy("../mod_Parameters.F90","JHUGenerator/mod_Parameters.F90")
     #replace mod_Parameters.F90 to modify widths of top and W	
      ...
      shutil.rmtree("JHUGenerator.v7.5.0")
      shutil.rmtree("JHUGenLexicon")
      shutil.rmtree("lib")
  ...
  with cd(os.path.join(JHUbasedir, "JHUGenerator")), open("../JHUGen.input") as f:
    if args.vbf_offshell:
      ...
      shutil.rmtree("data/")
    os.chdir(basedir)
    subprocess.check_call(["tar", "czf", "%s_JHUGen75.tar.gz" % (args.name),args.name+"_JHUGen"])
    #shutil.rmtree(JHUbasedir, ignore_errors=True)
